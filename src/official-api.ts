...
export class WeComOfficialAPI {
  ...
  async downloadMedia(
    ...
    const accessToken = await accessTokenManager.getAccessToken(corpid, corpsecret);
    ...
    if (!response.ok) {
      ...
      throw new Error(`Download media failed: ${response.status} ${response.statusText} ${text}`);
    }

    const contentType = response.headers.get("content-type") || undefined;
    const ab = await response.arrayBuffer();
    return { data: Buffer.from(new Uint8Array(ab)), contentType };
  }
}
