...
export class SimpleWecomClient {
  ...
  async sendMessage(userId: string, message: SimpleWecomMessage, config: {
    ...
    if (config.corpid && config.corpsecret && config.agentid) {
      try {
        // 如果有 imagePath：上传图片并发送 image
        if (message.imagePath) {
          const buf = await fs.readFile(message.imagePath);
          const up = await wecomOfficialAPI.uploadMedia(
            config.corpid,
            config.corpsecret,
            "image",
            buf,
          ...
          const result = await wecomOfficialAPI.sendMessage(config.corpid, config.corpsecret, payload);
          console.log("企业微信官方API发送图片成功:", result);
          return;
        }
